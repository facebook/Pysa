...
let test_of_expression _ =
  ...
  let assert_of_expression expression expected =
    assert_equal
      ...
      (AccessPath.of_expression
         ~self_variable:(Some (AccessPath.Root.Variable "self"))
         ~root_of_identifier:(fun ~location:_ ~identifier -> AccessPath.Root.Variable identifier)
         expression)
  in
  ...
  assert_of_expression
    ...
    (Some
       {
         AccessPath.root = AccessPath.Root.Variable "self";
         path = [Abstract.TreeDomain.Label.Index "b"];
       });
