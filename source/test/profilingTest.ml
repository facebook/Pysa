...
let with_global_state_restored ~f =
  let global_state = PyreProfiling.GlobalState.get () in
  Exn.protect ~f ~finally:(fun () -> PyreProfiling.GlobalState.restore global_state)
...
let test_event_format context =
  with_global_state_restored ~f:(fun () ->
